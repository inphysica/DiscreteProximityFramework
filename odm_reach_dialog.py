...
SETTINGS_KEY = 'DiscreteProximityFramework/odm_reach_path'


class ODMReachDialog(QDialog, FORM_CLASS):
    ...
    def _on_ok(self):
        ...
        settings = QSettings()
        settings.setValue('DiscreteProximityFramework/odm_reach_path', self.fileSelector.filePath())
        last_odm_path = settings.value('DiscreteProximityFramework/odm_reach_path','')

        speed_raw = settings.setValue('DiscreteProximityFramework/ODMReach_Speed',self.speedDial.value())
        max_duration_raw = settings.setValue('DiscreteProximityFramework/ODMReach_MaxDuration',self.MaxDurationDial.value())
        max_distance_raw = settings.setValue('DiscreteProximityFramework/ODMReach_MaxDistance',self.MaxDistanceDial.value())
    ...
    def _log_settings(self):
        ...
        try:
            settings = QSettings()
            # Try to list at least the odm_reach_path key and any keys under our plugin root
            try:
    ...
    def prep_Defaults(self):
        ...
        settings = QSettings()
        last_odm_path = ""
        if settings.contains('DiscreteProximityFramework/odm_reach_path'):
            last_odm_path = settings.value('DiscreteProximityFramework/odm_reach_path','')
            if os.path.exists(last_odm_path):
                ...
                last_odm_path = ""
                settings.setValue('DiscreteProximityFramework/odm_reach_path', '')

        speed_raw = settings.value('DiscreteProximityFramework/ODMReach_Speed',4.5)
        max_duration_raw = settings.value('DiscreteProximityFramework/ODMReach_MaxDuration',20)
        max_distance_raw = settings.value('DiscreteProximityFramework/ODMReach_MaxDistance',1.5)
...
def sub_Export_GeoJSON(self, distancemap, origins):
    ...
    output_filename = f"{base_name}_odm_reach_{timestamp}.geojson"
